<?php 
	...
	if (isset($_POST['answer'])) {
		...
		if (isset($_COOKIE['_aun']) && isset($_COOKIE['_apw'])) {
			...
			if (strtolower($answer) == "high fidelity") {

				$mysqli = mysqli_connect("localhost","root","root","grail");
				$username = $mysqli->real_escape_string($_COOKIE['_aun']);
				$points = $mysqli->query("SELECT * FROM `points` WHERE `point_name` = 'three_part_two'");
				...
				if (getPartStatus('three_part_two') == 1) {
					$mysqli->query("UPDATE `points` SET `points_awarded`= '$new_award' WHERE `point_name` = 'three_part_two'");
					$mysqli->query("UPDATE `players` SET `three_part_two` = '$points_awarded' WHERE `username` = '$username'");
					updateTotalScore();
